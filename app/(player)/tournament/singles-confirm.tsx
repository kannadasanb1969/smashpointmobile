    ...
    return (
      <ScreenContainer dark>
        <View style={s.successBody}>
          ...
          <Text style={s.successCenter}>Singles · {categoryName}</Text>
          <PrimaryButton
      ...
      <ScrollView contentContainerStyle={s.page} style={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.body}>
          ...
          <View style={s.summaryPanel}>
            ...
            <SummaryRow icon="people" label="CATEGORY" text={`Category: ${categoryName || ''}`} />
            <SummaryRow icon="shuttle" label="EVENT" text={`Event: ${eventType || 'Singles'}`} />

            <Pressable
              disabled={busy}
              onPress={submit}
              style={[s.confirmButton, busy && s.disabled]}
            >
              <View style={s.confirmIcon}>
                <TournamentIcon name="check" size={14} />
              </View>
              <Text style={s.confirmText}>{busy ? 'Checking…' : 'Confirm Registration'}</Text>
            </Pressable>
...
function SummaryRow({ icon, label, text }: { icon: 'trophy' | 'people' | 'shuttle'; label: string; text: string }) {
  return (
    <View style={s.row}>
