...
export default function Create() {
  ...
  return (
    <ScreenContainer>
      <ScrollView>
        <Text style={s.title}>Create Friendly Match</Text>
        <TextInput
          placeholder="Title"
          value={v.title}
        ...
        <TextInput
          ...
          value={v.maxPlayers}
          onChangeText={(x) => setV({ ...v, maxPlayers: x })}
          style={s.input}
          keyboardType="number-pad"
        />
        <Text
          onPress={() => {
            const eventType = v.eventType === 'SINGLES' ? 'DOUBLES' : 'SINGLES';
        ...
        <Text
          onPress={() => setV({ ...v, format: v.format === 'LEAGUE' ? 'KNOCKOUT' : 'LEAGUE' })}
          style={s.choice}
        >
          Format: {v.format}
        </Text>
        ...
        <PrimaryButton
          ...
          title={m.isPending ? 'Creating…' : 'Create'}
          onPress={() => {
...
const s = StyleSheet.create({
  ...
  input: { backgroundColor: colors.white, borderRadius: 12, padding: 14, marginTop: 12 },
  choice: {
